#!/usr/bin/env python3
"""
Build a MethodKG GraphSAGE structural-only award graph.
...
Outputs:
  graphsage_award_graph.pt
  graphsage_node_index.csv
  graphsage_edge_summary.csv
  graphsage_feature_manifest.csv
  graphsage_build_summary.json
  graphsage_benchmark_award_match_report.csv
  graphsage_benchmark_added_from_benchmark_only.csv
"""
...
def merge_benchmark_metadata(
    ...
    if outdir is not None:
        pd.DataFrame(report_rows).to_csv(outdir / "graphsage_benchmark_award_match_report.csv", index=False, encoding="utf-8-sig")
        ...
        missing_added[missing_cols].to_csv(outdir / "graphsage_benchmark_added_from_benchmark_only.csv", index=False, encoding="utf-8-sig")
...
def encode_targets_and_splits(df: pd.DataFrame, outdir: Path) -> Dict[str, Dict[str, int]]:
    ...
    with open(outdir / "graphsage_label_maps.json", "w", encoding="utf-8") as f:
...
def main() -> None:
    ...
    parser.add_argument("--awards", default=None, help="cleaned_nsf_awards_2000_2025.csv. Defaults to data/processed discovery.")
    parser.add_argument("--benchmark", default=None, help="methodkg_labeled_benchmark_v3_modeling.csv or benchmark_v3.zip. Defaults to data/benchmark discovery.")
    parser.add_argument("--award_pi_edges", default="", help="Optional award_pi_edges.csv from cleaning pipeline")
    parser.add_argument("--outdir", default=None, help="Output directory. Defaults to artifacts/graphs/graphsage_data_v2")
    parser.add_argument("--overwrite", action="store_true", help="Delete the output directory before writing new graph artifacts.")
    ...
    outdir = resolve_output_path(args.outdir, repo_root, repo_root / "artifacts" / "graphs" / "graphsage_data_v2")
    ...
    awards = add_basic_columns(read_csv_flexible(awards_path))
    awards = awards.drop_duplicates(subset=["award_id_key"]).reset_index(drop=True)
    benchmark = add_basic_columns(read_csv_flexible(benchmark_path))
    ...
    torch.save(data, outdir / "graphsage_award_graph.pt")
    ...
    node_index.to_csv(outdir / "graphsage_node_index.csv", index=False, encoding="utf-8-sig")

    pd.DataFrame(edge_summary).to_csv(outdir / "graphsage_edge_summary.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame(feature_manifest).to_csv(outdir / "graphsage_feature_manifest.csv", index=False, encoding="utf-8-sig")
    ...
    with open(outdir / "graphsage_build_summary.json", "w", encoding="utf-8") as f:
