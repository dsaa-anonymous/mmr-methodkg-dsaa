#!/usr/bin/env python3
"""
Train a SimTeG-style GraphSAGE text+graph model on the MethodKG award graph.

This script expects the output of build_simteg_graphsage_graph.py. It trains on
benchmark award nodes only, but message passing can use the full unlabeled award
projection graph. It uses precomputed award text embeddings plus optional structural features; it uses no raw candidate/annotation flags.
"""
...
def load_data(graph_dir: Path):
    data = torch.load(graph_dir / "simteg_graphsage_award_graph.pt", map_location="cpu")
    node_index = pd.read_csv(graph_dir / "simteg_graphsage_node_index.csv", low_memory=False, dtype=str, encoding="utf-8-sig")
    with open(graph_dir / "simteg_graphsage_label_maps.json", "r", encoding="utf-8") as f:
...
def main() -> None:
    ...
    parser.add_argument("--graph_dir", required=True, help="Directory created by build_simteg_graphsage_graph.py")
    ...
    torch.save(model.state_dict(), outdir / "simteg_graphsage_model.pt")
    with open(outdir / "run_summary.json", "w", encoding="utf-8") as f:
        json.dump(metrics, f, indent=2)

    print("Wrote SimTeG-GraphSAGE results to", outdir.resolve())
