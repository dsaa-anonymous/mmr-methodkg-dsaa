#!/usr/bin/env python3
"""Build a MethodKG heterogeneous graph for text+HGT (TG4).
...
It is designed for MethodKG TG4 text+graph experiments. Award nodes receive
precomputed text embeddings, optionally concatenated with structural metadata.
Labels are attached only to award nodes from the benchmark file.
...
from path_utils import default_text_hgt_embeddings, default_text_hgt_graph_dir, discover_award_pi_edges, discover_awards, discover_benchmark, find_repo_root, read_csv_or_zip, reset_dir_if_overwrite, resolve_existing_path, resolve_output_path, write_resolved_paths
...
def award_id_match_key(award_id: str) -> str:
    aid = normalize_award_id(award_id)
    if not aid:
        return ""
    return aid.lstrip("0") or "0"


def load_text_embedding_matrix(text_embeddings_path: str | Path, award_ids: List[str]) -> Tuple[np.ndarray, List[str], int]:
    """Load text embeddings and align them to award_ids. Missing rows get zeros.

    Accepts either methodkg_simteg_text_embeddings.csv, methodkg_text_embeddings.csv,
    or any CSV with award_id plus numeric embedding columns. Matching is robust to
    leading zeros via award_id_key.
    """
    emb_path = Path(text_embeddings_path)
    if not emb_path.exists():
        raise FileNotFoundError(f"Text embeddings file not found: {emb_path}")
    emb = read_csv_or_zip(emb_path, low_memory=False, encoding="utf-8-sig")
    if "award_id" not in emb.columns:
        raise ValueError("Text embeddings CSV must contain award_id column")
    emb["award_id"] = emb["award_id"].apply(normalize_award_id)
    if "award_id_key" not in emb.columns:
        emb["award_id_key"] = emb["award_id"].apply(award_id_match_key)
    else:
        emb["award_id_key"] = emb["award_id_key"].fillna(emb["award_id"]).apply(award_id_match_key)

    non_feature = {"award_id", "award_id_key", "benchmark_id", "text", "title_clean", "abstract_clean"}
    candidate_cols = [c for c in emb.columns if c not in non_feature]
    numeric_cols = []
    for c in candidate_cols:
        converted = pd.to_numeric(emb[c], errors="coerce")
        if converted.notna().any():
            emb[c] = converted
            numeric_cols.append(c)
    if not numeric_cols:
        raise ValueError("No numeric embedding columns found in text embeddings CSV")

    emb = emb[["award_id_key"] + numeric_cols].drop_duplicates("award_id_key", keep="first")
    lookup = emb.set_index("award_id_key")
    keys = [award_id_match_key(aid) for aid in award_ids]
    aligned = lookup.reindex(keys)
    missing = int(aligned[numeric_cols].isna().all(axis=1).sum())
    mat = aligned[numeric_cols].fillna(0.0).to_numpy(dtype=np.float32)
    return mat, numeric_cols, missing
...
def read_csv(path: str | Path) -> pd.DataFrame:
    return read_csv_or_zip(path, dtype=str, low_memory=False, encoding="utf-8-sig")
...
def main():
    ...
    ap.add_argument("--awards", default=None, help="cleaned_nsf_awards_2000_2025.csv. Defaults to data/processed/.../cleaned_nsf_awards_2000_2025.csv")
    ap.add_argument("--benchmark", default=None, help="Benchmark CSV/zip. Defaults to data/benchmark v3.")
    ap.add_argument("--award_pi_edges", default=None, help="Optional award_pi_edges.csv. Defaults to data/edges/award_pi_edges.csv if present.")
    ap.add_argument("--text_embeddings", default=None, help="CSV with award_id and text embedding columns. Defaults to artifacts/features/text_hgt_embeddings_scibert_v1/methodkg_simteg_text_embeddings.csv")
    ap.add_argument("--outdir", default=None, help="Output directory. Defaults to artifacts/graphs/text_hgt_data_scibert_v1")
    ...
    ap.add_argument("--drop_text_length_features", action="store_true", help="Remove title/abstract word-count features")
    ap.add_argument("--award_feature_mode", choices=["text_plus_structural", "text_only"], default="text_plus_structural", help="Use text embeddings alone or concatenate text embeddings with structural award metadata")
    ap.add_argument("--overwrite", action="store_true", help="Delete the output directory before rebuilding graph artifacts.")
    args = ap.parse_args()
    ...
    args.text_embeddings = str(resolve_existing_path(args.text_embeddings, repo_root) if args.text_embeddings else resolve_existing_path(default_text_hgt_embeddings(repo_root), repo_root))
    outdir = resolve_output_path(args.outdir, repo_root, default_text_hgt_graph_dir(repo_root))
    ...
    awards_raw = read_csv(args.awards)
    ...
    award_pi_path = Path(args.award_pi_edges) if args.award_pi_edges else None
    person_values = set()
    ...
    text_mat, text_feature_cols, missing_text_embeddings = load_text_embedding_matrix(args.text_embeddings, award_items)
    ...
    structural_mat = np.vstack(award_feats).T.astype(np.float32) if award_feats else np.zeros((n_awards, 0), dtype=np.float32)
    if args.award_feature_mode == "text_only":
        award_x = text_mat.astype(np.float32)
        feat_cols = [f"TEXT::{c}" for c in text_feature_cols]
    else:
        award_x = np.hstack([text_mat.astype(np.float32), structural_mat])
        feat_cols = [f"TEXT::{c}" for c in text_feature_cols] + feat_cols
    if award_x.shape[1] == 0:
        award_x = np.zeros((n_awards, 1), dtype=np.float32)
        feat_cols = ["constant"]
    data["award"].x = torch.tensor(award_x, dtype=torch.float32)
    ...
    award_node_index.to_csv(outdir / "text_hgt_award_node_index.csv", index=False, encoding="utf-8-sig")

    # Entity maps.
    def save_map(ntype, items):
        pd.DataFrame({"node_idx": range(len(items)), "node_id": items}).to_csv(outdir / f"text_hgt_{ntype}_node_index.csv", index=False, encoding="utf-8-sig")
    ...
    pd.DataFrame(edge_rows).sort_values(["src_type", "relation", "dst_type"]).to_csv(outdir / "text_hgt_edge_summary.csv", index=False, encoding="utf-8-sig")

    feature_manifest = []
    award_feature_desc = "|".join(feat_cols[:40]) + ("|..." if len(feat_cols) > 40 else "")
    feature_manifest.append({"node_type": "award", "feature_dim": int(data["award"].x.size(1)), "features": award_feature_desc, "num_text_embedding_features": int(len(text_feature_cols)), "missing_text_embeddings": int(missing_text_embeddings), "award_feature_mode": args.award_feature_mode})
    ...
    pd.DataFrame(feature_manifest).to_csv(outdir / "text_hgt_feature_manifest.csv", index=False, encoding="utf-8-sig")
    ...
    match_report.to_csv(outdir / "text_hgt_benchmark_award_match_report.csv", index=False, encoding="utf-8-sig")
    ...
    summary = {
        ...
        "text_embedding_dim": int(len(text_feature_cols)),
        "missing_text_embeddings": int(missing_text_embeddings),
        "award_feature_mode": args.award_feature_mode,
        "include_award_amount": bool(args.include_award_amount),
        "drop_text_length_features": bool(args.drop_text_length_features),
        "text_embeddings_file": str(args.text_embeddings),
        "note": "TG4 text+HGT graph. Award nodes use text embeddings, optionally concatenated with structural metadata.",
    }
    with open(outdir / "text_hgt_build_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2)

    torch.save(data, outdir / "text_hgt_heterodata.pt")
    # Save benchmark table used by runner.
    award_node_index.to_csv(outdir / "text_hgt_benchmark_table.csv", index=False, encoding="utf-8-sig")

    print("Wrote text+HGT graph to", outdir.resolve())
