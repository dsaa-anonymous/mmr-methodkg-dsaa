#!/usr/bin/env python3
"""Train structural-only HGT on MethodKG heterogeneous graph.

Predicts labels for award nodes from the 2,500-row benchmark using the split
columns produced by MethodKG benchmark v2.
"""
...
class HGTStructural(torch.nn.Module):
...
def main():
    ...
    data = torch.load(graph_dir / "hgt_heterodata.pt", map_location="cpu", weights_only=False)
    bench = pd.read_csv(graph_dir / "hgt_benchmark_table.csv", dtype=str, low_memory=False, encoding="utf-8-sig")
    ...
    torch.save(best_state, outdir / "hgt_model.pt")
    summary = {
        "target": args.target,
        "split_col": args.split_col,
        ...
        "note": "Structural-only transductive HGT baseline. No award text embeddings used.",
    }
